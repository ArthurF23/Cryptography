...
My current issue with the code is the example was made to only take a specific amount of characters at a time which would be constants in the code, so I have to fix that. In that problem has arisen problems with the encrypting and decrypting, whatever I am doing to the data is making it malformed or something as it surely is not working properly. I am modifying all the code from them and I am trying to make work arounds. As I get better with understanding it, performance will improve and the code will be much cleaner.